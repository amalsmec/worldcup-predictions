...
async function loadDb() {
  if (process.env.NETLIFY || process.env.GE_NETLIFY) {
    if (memoryDb) return memoryDb;
    try {
      const { getStore } = require('@netlify/blobs');
      const store = getStore('worldcup-db');
      const raw = await store.get('database.json');
      if (raw) {
      ...
      console.warn('Netlify Blobs failed to load database, falling back to empty in-memory DB:', e.message);
      memoryDb = {
        users: [],
        predictions: [],
        daily_predictions: [],
...
async function saveDb() {
  if (process.env.NETLIFY || process.env.GE_NETLIFY) {
    try {
      const { getStore } = require('@netlify/blobs');
      const store = getStore('worldcup-db');
      await store.set('database.json', JSON.stringify(memoryDb, null, 2));
    } catch (e) {
      console.error('Netlify Blobs failed to save database:', e.message);
    }
  } else {
    fs.writeFileSync(jsonFilePath, JSON.stringify(memoryDb, null, 2));
  }
...
function initDb() {
  return new Promise(async (resolve, reject) => {
    if (process.env.NETLIFY || process.env.GE_NETLIFY) {
      console.log('Running on Netlify. Forcing JSON database storage over Netlify Blobs.');
